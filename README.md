...
runs, about 4000 tests, and the slide decks under `docs/presentations/`.
...
`requirements.txt` pins `torch==2.11.0`, and the reference machine runs the `cu130`
build. If your driver needs a different CUDA build, install torch from the PyTorch
index first and then run the requirements file, which will leave the existing torch
alone.
...
Around 3950 tests. Anything that needs the real SAR stack is marked `real_data` and
skips itself when `test_data/` is missing, so a clean clone should come out green. That
run is your proof the environment is correct.
...
`docs/presentations/` is the tracked copy of the decks. The 26 result figures they use
...
tectonic full_project_story.tex      # the full deck, 147 pages
...
directory carries the deck structure and the design log.

The working copy at `presentations/` is gitignored and reads its figures straight out of
`results/`. The two copies are synchronised by hand.
