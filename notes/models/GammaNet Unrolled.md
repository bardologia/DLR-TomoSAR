...
Per pixel $p$, the forward model maps the reflectivity profile $\mathbf{s}_p \in \mathbb{R}_{\ge 0}^N$ on the elevation grid to complex coherence measurements over $T$ tracks through the steering operator $\mathbf{A}_p$ with entries $e^{i k_{z,t}(p) z_n} \, dz$ — the same operator the physics losses apply per track (`PhysicalLoss.synthesise_track`); `TomoOperator` builds it once per forward pass as a per-pixel steering tensor, used for the synthesiser's forward projection and the one-off adjoint backprojection $\mathbf{A}^H\mathbf{y}$. The per-iteration data-fidelity gradient never re-applies the operator pair: on the uniform height grid $\mathrm{Re}[\mathbf{A}^H\mathbf{A}]$ is Toeplitz per pixel, so `TomoOperator.gram` applies it as a per-pixel 1-D convolution (grouped depthwise conv) with the kernel $c(d) = \sum_t \cos(k_{z,t}\, dz\, d)$ of length $2N-1$, and the gradient is $(\mathbf{A}^H\mathbf{y} - dz\,c \circledast \mathbf{s})$ normalised as before; a non-uniform axis is rejected loudly since the Toeplitz identity depends on constant spacing. GammaNet inverts it by unrolling:
...
Entry point: `main/training/train_unrolled.py`, configured by `UnrolledEntryConfig` (`configuration/training/unrolled.py`) via `ConfigCli`. Following the exposure convention, the training knobs live in a slim `UnrolledTrainingConfig` containing exactly the fields the trainer reads; `ConfigFactory.unrolled_trainer_config` resolves them into an `UnrolledTrainerConfig`, the component bundle every family hands to `BaseTrainer`. `UnrolledTrainer` **subclasses `BaseTrainer`** and overrides only what is unrolled-specific: `_build_param_groups`, `_build_criterion` (the masked L1/MSE curve loss with peak-MAE as its physical metric), `_compute_loss` (measurement synthesis then GammaNet forward), and `_log_epoch_metrics` (peak MAE to the run log, since there is no TensorBoard writer). Everything else — epoch/step structure, gradient accumulation, `GradientClipper`, `EarlyStopping`, `WeightEma`, `Checkpoint`, resumable `TrainerState`, epoch snapshots, `VramReservation`, `ResourceMonitor` — comes from the shared loop. AMP stays off (`use_amp` is pinned to `False` in the factory) because the forward operator is complex-valued, and the cosine schedule and linear warmup are likewise fixed for this family, so none of the three are exposed. Run layout: `logdir/<run>/{docs,meta,logs,best_model.pt,last.pt,training_summary.json}` with the model architecture persisted through `UnrolledModelConfigIO`; `training_summary.json` (loss history, best validation loss and epoch, test loss and peak MAE) is written by the pipeline and read back by unrolled inference. The script is registered in the webui: a Train-group variant ("Unrolled") in `ScriptCatalog`, an `ENTRY_OVERRIDES` mapping in `project_paths.py`, and a `launch_layout` page claiming every entry-config field; it is deliberately absent from `TensorboardManager.TRAINING_LOGDIRS` because the run carries no writer and so emits no event files.
