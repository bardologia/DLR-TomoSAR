...
`AttentionUNet` (`models/AttentionUNet.py`) extends the standard [[UNet]] by replacing direct skip connection concatenation with attention-gated skip connections ([[AttentionUNet_Oktay2018_1804.03999.pdf|Oktay et al., 2018]]). An attention gate learns to suppress spatially irrelevant activations in the skip features before merging them with the decoder.
...
This section records an equation-by-equation, figure-by-figure verification of `AttentionUNet` (`models/AttentionUNet.py`, builders in `configuration/models_config.py`) against the paper. The paper PDF is treated as ground truth. The reference architecture is 3D (volumetric CT); our implementation is the 2D adaptation for [[TomoSAR]] tomograms, so any pure $3D \rightarrow 2D$ change (e.g. trilinear $\rightarrow$ bilinear resampling, $\text{Conv3d} \rightarrow \text{Conv2d}$) is an accepted adaptation rather than a deviation.

**Gate correction (2026-06-04).** The attention gate was corrected on this date to the paper's coarse-grid (grid-attention) formulation. Prior to the fix, the gate computed coefficients at the fine skip resolution using the *upsampled* decoder feature as the gating signal and lacked the output transform. The corrected gate (i) takes the pre-upsample decoder feature as the gating signal $g$, (ii) projects the skip with a kernel-2 stride-2 convolution ($W_x$) so attention is computed on the coarse $H_g \times W_g$ grid as in Fig. 2 and Section 3.2 ("input feature-maps are downsampled to the resolution of gating signal"), (iii) bilinearly resamples $\alpha$ back to the skip grid (the Resampler block of Fig. 2), and (iv) applies an output transform $W$ ($1\times1$ conv + norm) after the multiplicative gating. The verification below confirms this matches the paper.

### Component-verdict table

| # | Component | Paper reference | Code reference | Verdict |
|---|---|---|---|---|
| 1 | Backbone encoder/decoder (double $3\times3$ conv + ReLU per scale, factor-2 rescaling) | Fig. 1; Sec. 2 | `AttentionUNet.py:11-47, 152-219` | MATCH |
| 2 | Gate signal $g$ = coarse decoder feature (pre-upsample) | Fig. 1, Fig. 2; Sec. 3.2 | `AttentionUNet.py:242` (gate called before `upsample`) | MATCH |
| 3 | $W_g$ ($1\times1$ conv, to $F_{int}$) on $g$ | Eq. 2; Fig. 2 | `AttentionUNet.py:61-69` | MATCH |
| 3 | $W_x$ ($1\times1$ conv on $x^l$, feature-maps downsampled to gate grid) | Eq. 2; Sec. 3.2; Fig. 2 | `AttentionUNet.py:70-79` (kernel-2 stride-2 conv) | ACCEPTED ADAPTATION |
| 4a | $\sigma_1$ = ReLU on $W_x^T x + W_g^T g$ | Eq. 1 | `AttentionUNet.py:98, 114` | MATCH |
| 4b | $\psi$ ($1\times1$ conv to single channel) | Eq. 1; Fig. 2 | `AttentionUNet.py:80-86` | MATCH |
| 4c | $\sigma_2$ = sigmoid $\rightarrow \alpha \in [0,1]$ | Eq. 2; Sec. 3.2 | `AttentionUNet.py:87` | MATCH |
| 5 | Resampling of $\alpha$ back to $x^l$ grid | Fig. 2 (Resampler, trilinear) | `AttentionUNet.py:115-120` (bilinear) | ACCEPTED ADAPTATION |
| 6 | Output transform $W$ ($1\times1$ conv + norm) on gated skip | Fig. 2 (right recombination block) | `AttentionUNet.py:89-97, 121` | MATCH |
| 7 | Gated skip enters decoder via concatenation before refinement conv | Fig. 1 (concatenation); Sec. 3.2 | `AttentionUNet.py:245-246` | MATCH |
| 8 | Downsampling (max-pool / 2), upsampling (/2) of backbone | Fig. 1 legend | `AttentionUNet.py:166`, `models_config.py:53-75` | MATCH |
| 9 | Deep supervision | Sec. 3 (Implementation Details) | not implemented | DEVIATION (minor) |
| 10 | Multiplicative gating $\hat{x}_{i,c}^l = x_{i,c}^l \cdot \alpha_i^l$ | Sec. 3.2 | `AttentionUNet.py:121` | MATCH |
