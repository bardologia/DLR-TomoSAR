...
`UNETR` (`models/UNETR.py`) uses a pure Vision Transformer (ViT) encoder with a CNN decoder that receives skip connections from intermediate ViT layer outputs ([[UNETR_Hatamizadeh2021_2103.10504.pdf|Hatamizadeh et al., 2022]]).
...
The default `embedding_dim = 544` is divisible by `transformer_heads = 8`.

---

## Paper fidelity

*Review date: 2026-06-04. Ground truth: Hatamizadeh et al., "UNETR: Transformers for 3D Medical Image Segmentation", arXiv:2103.10504 (Sec. 3.1 Architecture, Fig. 2, Eq. 1-6; [[UNETR_Hatamizadeh2021_2103.10504.pdf|PDF]]). Code under review: `models/UNETR.py`, `configuration/models_config.py` (`UNETRConfig`). The original is 3D ($H\times W\times D$, $P^3$ patches, $3\times3\times3$ convs, Deconv $2\times2\times2$); our implementation is the 2D analog ($H\times W$, $p^2$ patches, $3\times3$ convs, Deconv $2\times2$) and is judged on the 2D analog of each structure.*

**Overall verdict: faithful 2D adaptation.** Patch-direct embedding (no CNN encoder), the pre-norm ViT encoder (Eq. 1-3), the $\{z_3,z_6,z_9,z_{12}\}$ even-spacing skip rule, the per-skip deconv-step counts (3/2/1), the bottleneck-deconv-then-merge wiring, the multi-scale concat-then-conv decoder, the full-resolution input-stem skip, and the $1\times1$ output head all reproduce Fig. 2. The projection / blue-block composition was corrected on 2026-06-04 (see item 5b): each deconv is now followed by a $3\times3$ conv + norm + act and the front projection is a $3\times3$ conv, matching the Fig. 2 legend. One minor deviation remains: the patch embedding adds a `LayerNorm` not present in Eq. 1.

| # | Component (paper ref) | Code site | Verdict |
|---|---|---|---|
| 1 | Direct patch embedding, no CNN encoder; linear projection of non-overlapping $p\times p$ patches (Eq. 1) | `UNETR.py:182-204`, `372` | Match (2D analog) |
| 1b | Learned 1D positional embedding, no class token (Eq. 1) | `UNETR.py:247-248`, `373` | Match |
| 1c | Extra `LayerNorm` on patch embedding (not in Eq. 1) | `UNETR.py:197,203` | Deviation (minor) |
| 2 | ViT block: pre-norm, $z'=\mathrm{MSA}(\mathrm{LN}(z))+z$, $z=\mathrm{MLP}(\mathrm{LN}(z'))+z'$, GELU MLP (Eq. 2-3) | `UNETR.py:142-178` | Match |
| 2b | Scaled dot-product MSA, $K_h=K/n$ scaling (Eq. 4-6) | `UNETR.py:103-138` | Match |
| 3 | Even-spaced skip taps $z_3,z_6,z_9,z_{12}$ via quartile rule; final LN on deepest tap | `UNETR.py:266-272`, `377-381` | Match (2D analog) |
| 4 | Reshape each tapped sequence to a 2D grid $(B,d,H/p,W/p)$ | `UNETR.py:220-222`, `381` | Match (2D analog) |
| 5 | Per-skip deconv counts: $z_9$=1, $z_6$=2, $z_3$=3 | `UNETR.py:276-301` | Match (count) |
| 5b | Blue-block composition: each deconv followed by Conv $3\times3$+BN+ReLU (Fig. 2 legend) | `UNETR.py:53-99` | Match (corrected 2026-06-04) |
| 6 | Bottleneck $z_{12}$ projected then deconv by 2, merged with $z_9$ (Fig. 2) | `UNETR.py:302-309`, `391,394-404` | Match (2D analog) |
| 7 | Decoder per scale: concat then conv blocks then deconv | `UNETR.py:321-348`, `393-404` | Accepted adaptation |
| 8 | Full-resolution input stem: raw input through conv blocks, concat at last stage (Fig. 2 bottom) | `UNETR.py:312-319`, `399-400` | Match (2D analog) |
| 9 | Output head $1\times1$ conv | `UNETR.py:360-364`, `408` | Match (2D analog) |
| 10 | Resolution / channel bookkeeping of every merge | `UNETR.py:321-348`, `393-407` | Match |

**Encoder (items 1-2).** The patch embedding is a strided $\mathrm{Conv2d}$ with kernel = stride = $p$, the exact equivalent of the per-patch linear projection $E$ in Eq. 1; the learned positional table (`UNETR.py:247`) is added with no class token, matching the paper's explicit note that the `[class]` token is dropped for segmentation. The transformer block (`UNETR.py:173-177`) implements the pre-norm form of Eq. 2-3 verbatim — $\mathrm{LN}\to\mathrm{MSA}\to$ residual, then $\mathrm{LN}\to\mathrm{MLP}\to$ residual — with a GELU two-layer MLP (`ffn_activation = "gelu"`). The attention (`UNETR.py:124-138`) is standard scaled dot-product with the $1/\sqrt{K_h}$ scaling of Eq. 4. The paper fixes $L=12$, $K=768$; here $L$ and $d$ are hyperparameters (default $L=8$, $d=544$) and out of scope.

**Skip rule (item 3).** The paper taps the fixed layers $\{3,6,9,12\}$, i.e. the quartiles of $L=12$. The code generalises this to $\{L/4,\,L/2,\,3L/4,\,L\}$ (0-based: `total_layers//4 - 1`, etc., `UNETR.py:267-272`), which reproduces $\{2,5,8,11\}$ (0-based for $\{3,6,9,12\}$) at $L=12$ and stays evenly spaced for any $L$. The final transformer `LayerNorm` is applied only to the deepest tap (`UNETR.py:380`), consistent with the paper applying Norm at the encoder output / bottleneck; the three shallower taps are forwarded un-normed, which is the standard reading of Fig. 2. Match.

**Projection / blue block (items 4-5).** Each tapped sequence is reshaped to the grid (`tokens_to_feature_map`, `UNETR.py:220-222`) and passed through a `ProgressiveProjectionHead`. The number of upsample steps (3 for $z_3$, 2 for $z_6$, 1 for $z_9$, 0 for the $z_{12}$ bottleneck) exactly matches the blue-block counts in Fig. 2. **Correction (2026-06-04).** The head was previously a single $1\times1$ conv + norm + act front projection followed by bare `[ConvTranspose2d, Norm, Act]` repeats, which dropped the post-deconv $3\times3$ convolutions of the paper's blue block. It now (`UNETR.py:64-95`) opens with a $3\times3$ conv + norm + act front projection (matching the paper's "consecutive $3\times3$ convolutional layers" projection) and repeats `[ConvTranspose2d, Conv $3\times3$, Norm, Act]` per upsample step, the 2D analog of the Fig. 2 legend "Deconv $2\times2\times2$, Conv $3\times3\times3$, BN, ReLU". The bottleneck head (`upsample_steps=0`) is now the $3\times3$ conv + norm + act front projection. Interface channel bookkeeping is unchanged — each head still emits its target `decoder_features[...]` width, so decoder wiring is untouched. Match.

**Bottleneck and decoder (items 6-7).** The $z_{12}$ tap is projected with 0 upsample steps (`UNETR.py:302-309`), staying at the $H/p$ grid; decoder step `idx0` then deconvs it $\times2$ and concatenates the $z_9$ projection (`UNETR.py:391,394-404`), reproducing the Fig. 2 "deconv bottleneck by 2, then merge with $z_9$" path. Each decoder level (`UNETR.py:327-348`) does ConvTranspose-up $\to$ resize-to-skip $\to$ concat $\to$ double-$3\times3$-conv ConvBlock. The paper draws the level as concat $\to$ yellow convs $\to$ green deconv, i.e. the deconv that the code places at the *start* of level $n{+}1$ the paper draws at the *end* of level $n$. This is the standard equivalent regrouping of the U-Net decoder ladder; the connectivity graph and channel flow are identical. Accepted adaptation. The yellow conv blocks (Conv $3\times3$ + BN + ReLU, $\times2$) are faithfully realised by `ConvBlock` (`UNETR.py:11-49`).

**Stem and output (items 8-9).** The raw input is processed by `input_skip_conv` (a double-$3\times3$ ConvBlock, `UNETR.py:312-319`) and concatenated at the final, full-resolution decoder stage (`UNETR.py:399-403`), matching the Fig. 2 bottom path (Input $\to$ two yellow convs at $H\times W\times64 \to$ concat before the output head). The output head is a $1\times1$ conv (`UNETR.py:360-364`); the paper's softmax is a loss-side detail and out of scope.
...
**Deviation summary.**
- *Blue-block $3\times3$ conv* — **resolved 2026-06-04.** Paper: Fig. 2 legend (blue block = Deconv + Conv $3\times3$ + BN + ReLU). Code: `UNETR.py:53-99` (`ProgressiveProjectionHead`). The head now interleaves a $3\times3$ conv + norm + act after every `ConvTranspose2d` and uses a $3\times3$ conv (+ norm + act) front projection; interface channels unchanged, decoder wiring untouched. No longer a deviation.
- *Patch-embedding `LayerNorm`* — minor (open). Paper: Eq. 1 (linear projection + pos embed only). Code: `UNETR.py:197,203`. Fix: drop `self.norm` (or accept as a benign modern-ViT stabiliser; no structural impact).
