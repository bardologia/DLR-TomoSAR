...
*Review date: 2026-06-04.* Ground truth: Lin et al., *Feature Pyramid Networks for Object Detection*, arXiv:1612.03144 (Sec. 3, Fig. 3) [[FPN_Lin2017_1612.03144.pdf|PDF]] and Kirillov et al., *Panoptic Feature Pyramid Networks*, arXiv:1901.02446 (Sec. 3.1, Fig. 3) [[PanopticFPN_Kirillov2019_1901.02446.pdf|PDF]]. Comparison against `models/FPNNet.py` and `configuration/models_config.py` (`FPNNetConfig`, build helpers).

### Verdict table

| # | Dimension | Verdict | Evidence |
|---|-----------|---------|----------|
| 1 | Bottom-up feature hierarchy, stride-2 between stages | MATCH | Lin Sec. 3 "scaling step of 2"; `FPNNet.py:59` `MaxPool2d(2)` per stage (strides $\{1,2,4,8\}$) |
| 2 | Lateral $1\times1$ conv on each bottom-up map | MATCH | Lin Fig. 3, Sec. 3; `FPNNet.py:72` |
| 3 | Top-down upsample $\times2$ + element-wise ADD | MATCH (add) / DEVIATION-minor (bilinear vs nearest) | Lin Sec. 3 "nearest neighbor upsampling", merge by addition; `FPNNet.py:102-103` bilinear interpolate + `lateral + top_down` |
| 4 | Post-merge $3\times3$ anti-aliasing conv per level | MATCH | Lin Sec. 3 "append a $3\times3$ convolution on each merged map"; `FPNNet.py:73,105` |
| 5 | Uniform pyramid channel width | MATCH | Lin Sec. 3 ($d{=}256$, uniform); Kirillov Sec. 3.1 "same channel dimension"; `FPNNet.py:72-73`, `pyramid_channels` |
| 6 | Semantic branch: depth-scaled upsampling stages, SUM merge | MATCH (corrected 2026-06-04) | Kirillov Sec. 3.1 + Fig. 3 (progressive 3x3+norm+act+2x stages, more for deeper levels); `FPNNet.py:11-39,76-79` `SegmentationBlock` now emits $N_i=\log_2(\text{stride}_i/\text{stride}_\text{target})$ interleaved stages ($N=0,1,2,3$), SUM merge retained |
| 7 | Fused map predicted ($1\times1$) and upsampled | MATCH (predict) / DEVIATION-minor (no final upsample) | Kirillov Sec. 3.1 final $1\times1$ + $4\times$ upsample; `FPNNet.py:88,118-119` |
| 8 | Index-0 full-resolution ($1/1$) level | ACCEPTED ADAPTATION | Papers start pyramid at $1/4$; code adds an identity-stride $P_1$-equivalent level (`FPNNet.py:59`) |
...
| 10 | Smooth/lateral layers carry no non-linearity | MATCH | Lin Sec. 4.1 "no non-linearities in these extra layers"; `FPNNet.py:72-73` bare convs |

### Prose

The **bottom-up pathway, lateral connections, additive merge, post-merge smoothing, and uniform channel width** are all faithful to Lin Sec. 3 and Fig. 3. The encoder reuses the pre-activation [[ResUNet]] `ResidualConvBlock`; that block gained a `stride` parameter on 2026-06-04 (default $1$), but `FPNNet` constructs it without passing `stride`, so every block stays stride-$1$ and spatial reduction is delegated entirely to the separate `MaxPool2d(2)` layers (`FPNNet.py:58-70`). This is consistent with Lin's explicitly *generic-backbone* assumption (Sec. 3, "independent of the backbone convolutional architecture"), so the construction remains paper-conformant.

Two **minor deviations** concern the top-down resampling: the paper specifies nearest-neighbour upsampling "for simplicity", whereas the code uses `mode="bilinear"` (`FPNNet.py:102`). Bilinear is a strictly richer interpolation and is the choice Panoptic FPN itself makes in the semantic branch, so this is a benign, well-motivated minor deviation. Relatedly, the final output is produced by the $1\times1$ head at the merge resolution with no subsequent upsample (`FPNNet.py:118-119`); because the merge resolution is already full input resolution (level 0 is $1/1$), this is correct for the adapted geometry and only a nominal difference from Kirillov's "$1\times1$ then $4\times$ upsample".

The semantic segmentation branch was a **structural deviation** that was **corrected on 2026-06-04**. Kirillov Sec. 3.1 / Fig. 3 prescribe a *depth-scaled* upsampling decoder: each FPN level is brought to the target scale by a sequence of upsampling stages, where a stage is $3\times3$ conv + norm + activation + $2\times$ bilinear, and **deeper (coarser) levels receive more stages** (in the paper the $1/32$ level gets three stages, $1/16$ two, $1/8$ one, $1/4$ a single conv with no upsample), after which the equal-scale maps are summed. The previous code applied an *identical* fixed-depth `SegmentationBlock` (a fixed `segmentation_convs` count of $3\times3$+norm+act, no interleaved upsampling) to every level and then resampled each result to the target size in a **single** `interpolate` call before summing; this collapsed the progressive multi-stage decode into one block plus one large-factor interpolation, removing the intermediate learned refinement at each $2\times$ step and not scaling processing with level depth.

**Correction applied (2026-06-04).** `SegmentationBlock` now takes an `n_stages` argument and emits $N_i = \log_2(\text{stride}_i/\text{stride}_\text{target})$ interleaved stages of ($3\times3$ conv → norm → act → $2\times$ bilinear upsample); the shallowest level ($N_i=0$) degenerates to `segmentation_convs` $3\times3$ conv → norm → act units with no upsample (`FPNNet.py:18-24`), generalising the single-conv $1/4$ level of Kirillov Fig. 3 to the configured per-stage conv count. With the encoder strides $\{1,2,4,8\}$ and the target at level 0, the four levels receive $N = 0,1,2,3$ stages. The per-level loop now reaches the common scale through these repeated $2\times$ stages; the residual `interpolate` survives only to absorb the floor-rounding mismatch produced by odd input sizes (e.g. a $70\times70$ input). The element-wise SUM merge is retained, matching the paper (Kirillov ablation Table 1d confirms sum over concat). The `segmentation_convs` hyperparameter is preserved with refined semantics — the number of $3\times3$ conv → norm → act units *within each stage* — leaving its tunable choices $\{1,2,3\}$ coherent. The bottom-up / lateral / top-down core is untouched; pyramid width and per-stage conv count remain hyperparameters. Verified by a smoke test (forward + backward at $(2,C,64,64)$ and $(2,C,70,70)$ with exact spatial-dim recovery, and $100\%$ parameter coverage by `get_param_groups`).
