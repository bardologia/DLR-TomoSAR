...
class Encoder(nn.Module):
    def __init__(self, input_channels: int, feature_sizes: list[int], dropout: float = 0.0):
        ...
        for feature_size in feature_sizes:
            self.conv_blocks.append(ConvBlock(channels, feature_size, dropout))
    ...
    def __init__(self, feature_sizes: list[int], dropout: float = 0.0):
        ...
        for index in range(len(feature_sizes) - 1):
            self.upsample_layers.append(
                nn.ConvTranspose2d(feature_sizes[index], feature_sizes[index + 1], kernel_size=2, stride=2)
            )
            self.conv_blocks.append(
                ConvBlock(feature_sizes[index], feature_sizes[index + 1], dropout)
            )
