...
class ResidualEncoderBlock(nn.Module):
    def __init__(self, input_channels: int, output_channels: int, dropout: float = 0.0,
        ...
        self.shortcut_path = nn.Sequential(
            nn.Conv2d(input_channels, output_channels, kernel_size=1, stride=2, bias=bias),
            build_norm2d(normalization, output_channels),
        )
...
class BottleneckDecoderBlock(nn.Module):
    def __init__(self, input_channels: int, output_channels: int, compression_ratio: int = 4,
        ...
        self.layers = nn.Sequential(
            nn.Conv2d(input_channels, compressed_channels, kernel_size=1, bias=bias),
            ...
            build_norm2d(normalization, compressed_channels),
            build_activation(activation),
            nn.Conv2d(compressed_channels, output_channels, kernel_size=1, bias=bias),
            build_norm2d(normalization, output_channels),
            build_activation(activation),
        )
...
class LinkNet(nn.Module):
    def __init__(self, config: LinkNetConfig | None = None):
        ...
        self.initial_conv = nn.Sequential(
            nn.Conv2d(config.in_channels, feature_sizes[0], kernel_size, stride=1, padding=padding, bias=config.conv_bias),
