...
class TuningOrchestrator:
    ...
    def _tune_model(self, model_name: str, tune_cfg) -> None:
        ...
        p1_counts = self._distribute_trials(tune_cfg.phase1.n_trials, n_gpus)
        self._create_study(model_name, 1, tune_cfg.phase1)
        self.logger.subsection(f"Phase 1 — {sum(p1_counts)} trials across {n_gpus} GPUs")
        ...
        FileIO.save_json(decoded_p1, best_p1_path, indent=2)
        ...
        p2_counts = self._distribute_trials(tune_cfg.phase2.n_trials, n_gpus)
        self._create_study(model_name, 2, tune_cfg.phase2)
        self.logger.subsection(f"Phase 2 — {sum(p2_counts)} trials across {n_gpus} GPUs")
