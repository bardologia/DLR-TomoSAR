...
class DatasetCreationPipeline:
    def __init__(
        self,
        config                 : DatasetCreationConfiguration,
    ...
    ) -> Tuple[TomoPatchDataset, Patcher]:
    ...
    def run(self) -> Tuple[DataLoader, DataLoader, DataLoader, dict[str, TomoPatchDataset]]:
        self.logger.section("[Dataset Creation Pipeline Execution]")

        train_ds, train_patcher = self._build_dataset("train")

        norm_stats = NormalizationStats.compute_from_dataset(
            dataset             = train_ds,
            logger              = self.logger,
            input_config        = self.config.input_config,
