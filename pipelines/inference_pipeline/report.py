...
class ReportAssembler:
    ...
    def _four_col_table(
        ...
        header : Tuple[str, str, str, str] = ("Metric", "Pred vs GT", "Pred vs Raw", "Description"),
    ) -> str:
    ...
    def _is_per_slice_ssim(k: str) -> bool:
        ...
        for prefix in (
            ...
            "elev_mae_raw_", "elev_rmse_raw_", "elev_r2_raw_", "elev_ce_raw_",
        ):
            if k.startswith(prefix) and k[len(prefix):].lstrip("-").isdigit():
                return True
        return False
    ...
    def _build_headline_metrics(self) -> List[str]:
        ...
        out.append(self._four_col_table([
            ("SSIM elev mean",    gm.get("ssim_gt_elev_mean"),    gm.get("ssim_raw_elev_mean"),    "H\u00d7W intensity-at-elevation-bin planes"),
            ("SSIM range mean",   gm.get("ssim_gt_range_mean"),   gm.get("ssim_raw_range_mean"),   "n_elev\u00d7H cross-sectional planes"),
            ("SSIM azimuth mean", gm.get("ssim_gt_azimuth_mean"), gm.get("ssim_raw_azimuth_mean"), "n_elev\u00d7W cross-sectional planes"),
        ]))
        out.append("")
    ...
    def _build_full_metrics(self) -> List[str]:
        ...
        for k, v in sorted(gm.items()):
            ...
            elif k.startswith("pixel_") and "_raw" not in k:
                groups["3.4 Per-pixel (GT)"][k] = v
            elif k.startswith("pixel_") and "_raw" in k:
                groups["3.5 Per-pixel (Raw)"][k] = v
            elif k.startswith("ssim_"):
                groups["3.6 SSIM summaries"][k] = v
            elif "_raw" in k:
                groups["3.3 Curve-level (Raw)"][k] = v
            else:
    ...
    def _build_figures(self) -> List[str]:
        ...
        out.append(
            "Each panel overlays the raw input (blue dotted), GT profile (black solid), "
        ...
        out.append("\n## 8. SSIM curves\n")
        out.append("SSIM plotted for every slice along each axis \u2014 blue: pred vs GT, red: pred vs Raw.\n")
        for key, title in (
        ...
        if "elev_metric_curves" in fp:
            ...
            out.append(
                ...
                "elevation bin. Blue: pred vs GT (Gaussian); red: pred vs Raw. "
