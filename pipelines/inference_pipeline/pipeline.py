...
from pipelines.inference_pipeline.metrics   import compute_global_metrics, select_pixels_by_metric
from pipelines.inference_pipeline.plots     import (
    apply_style,
    plot_elevation_intensity_slice,
...
from pipelines.inference_pipeline.report    import assemble_report, write_metrics_json
...
class InferencePipeline:
    ...
    def run(self) -> Path:
        ...
        apply_style(fig_dpi=cfg.fig_dpi, save_dpi=cfg.save_dpi)
        ...
        # Pre-compute slice indices so they are shared between SSIM metrics
        # and the figure-generation loop that follows.
        ...
        # Full-range index arrays for SSIM computation over every slice
        ...
        selected = select_pixels_by_metric(
            result.pixel_mse,
        ...
        for tag, pixels in (("best", selected["best"]), ("worst", selected["worst"]), ("random", selected["random"])):
            figure_paths[f"profiles_{tag}"] = plot_profile_panel(
        ...
        figure_paths["pixel_mse_map"] = plot_pixel_metric_map(
        ...
        figure_paths["pixel_r2_map"] = plot_pixel_metric_map(
        ...
        figure_paths["pixel_peak_map"] = plot_pixel_metric_map(
        ...
        figure_paths["metric_histograms"] = plot_metric_histogram(
            {
        ...
        figure_paths["param_maps"] = plot_param_maps(
        ...
        for s_idx, i in enumerate(slice_range_idx):
            ...
            figure_paths[tag] = plot_tomogram_slice(
                ...
                ssim_value = global_metrics.get(f"ssim_range_{s_idx}"),
            )
        ...
        for s_idx, i in enumerate(slice_az_idx):
            ...
            figure_paths[tag] = plot_tomogram_slice(
                ...
                cmap       = cfg.cmap_intensity,
                err_cmap   = cfg.cmap_error,
                ssim_value = global_metrics.get(f"ssim_azimuth_{s_idx}"),
            )
        
        for s_idx, i in enumerate(slice_elev_idx):
            tag = f"slice_elev_idx_{int(i)}"
            figure_paths[tag] = plot_elevation_intensity_slice(
                result.pred_curves, result.gt_curves, result.raw_curves,
                elev_idx   = int(i),
                x_axis     = x_axis_np,
                out_path   = figures_dir / f"{tag}.png",
        ...
        figure_paths["ssim_azimuth"] = plot_ssim_curves(
            global_metrics = global_metrics,
            axis           = "azimuth",
        ...
        figure_paths["ssim_elev"] = plot_ssim_curves(
            global_metrics = global_metrics,
            axis           = "elev",
