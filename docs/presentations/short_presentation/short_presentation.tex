...
\begin{frame}{Next steps}
  \begin{itemize}\setlength{\itemsep}{16pt}
    \item \textbf{Physical loss terms}: the five ranked terms and the first single-term $K{=}2$ sweep results on the next five frames; the Capon-cycle arm is still queued.
...
\begin{frame}{The single-term sweep --- every family helps low, two collapse high}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.54\textwidth}
      ...
      $\cdot$ full norm ladder $\cdot$ hv flips $\cdot$ patch $64{\times}32$ $\cdot$
      full stack $\to$ both trunks; loss $=$ param-L1 $+\,0.5\,$cosine curve $+$ one
      physics term at weight $w$; five seeded replicas per arm, held-out test. Cell
      $=$ grouped score: equal-weight mean of the five metric-group scores (curve
      error / variance explained / structural similarity / shape / peak location),
      min-max scaled \emph{within this 21-arm comparison} --- a rank statistic, no
      seed spread. \textbf{bold} $=$ best in row, marked only where best and worst
      differ by more than $5\%$ (every row clears it);
      \colorbox{accent!18}{\strut below baseline}; \colorbox{good!25}{\strut sweep
      best}. Val losses are not comparable across arms (each optimises a different
      composite).\par}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{The verdict --- physics terms buy tail insurance, not headline gains}
  ...
  \begin{columns}[T]
    \begin{column}{0.33\textwidth}
      \begin{itemize}\setlength{\itemsep}{6pt}
        ...
        \item[\textcolor{argB}{$\blacksquare$}] \textbf{The real effect is the
              ...
              cap the blow-ups and pull the mean positive.
    ...
    \begin{column}{0.65\textwidth}
      ...
      \setlength{\tabcolsep}{2pt}
      \renewcommand{\seedtag}{phyV}
      \begin{tabular}{@{}l ccccc@{}}
        \toprule
        metric & baseline & coh.\ 0.25 & mom.\ 0.05 & cov.\ 0.05 & pow.\ 0.05 \\
        \midrule
        \rowcolor{argA!14} curve $R^2$ $\uparrow$ & \seedpm{0.747}{.006} & \seedpm{0.755}{.007} & \seedpm{0.755}{.002} & \seedpm{0.748}{.001} & \seedpm{0.751}{.005} \\
        \rowcolor{argA!14} PSNR [dB] $\uparrow$   & \seedpm{52.63}{.10} & \seedpm{52.78}{.13} & \seedpm{52.76}{.04} & \seedpm{52.65}{.02} & \seedpm{52.70}{.08} \\
        \addlinespace[2pt]
        \rowcolor{argB!16} pixel $R^2$ mean $\uparrow$ & \seedpm{$-1.17$}{.79} & \seedpm{$-0.88$}{.53} & \seedpm{$+0.53$}{.19} & \seedpm{$\mathbf{+0.77}$}{.01} & \seedpm{$+0.73$}{.05} \\
        \rowcolor{argB!16} pixel $R^2$ $p_1$ $\uparrow$ & \seedpm{$-14.9$}{11.4} & \seedpm{$-12.4$}{4.0} & \seedpm{$-2.2$}{1.1} & \seedpm{$\mathbf{-0.5}$}{0.1} & \seedpm{$-0.9$}{0.4} \\
        \rowcolor{argB!16} pixel $R^2$ min $\uparrow$ & \seedpm{$-13.2$k}{3.3k} & \seedpm{$-12.2$k}{4.8k} & \seedpm{$-4.4$k}{1.2k} & \seedpm{$-4.2$k}{0.2k} & \seedpm{$\mathbf{-4.1}$k}{0.1k} \\
        \rowcolor{argB!16} cos med $\uparrow$ & \seedpm{0.965}{.004} & \seedpm{0.966}{.002} & \seedpm{0.966}{.002} & \seedpm{0.966}{.002} & \seedpm{0.966}{.001} \\
        \addlinespace[2pt]
        recall $\uparrow$      & \seedpm{0.901}{.001} & \seedpm{0.903}{.002} & \seedpm{0.902}{.001} & \seedpm{0.902}{.001} & \seedpm{0.900}{.001} \\
        precision $\uparrow$   & \seedpm{0.895}{.008} & \seedpm{0.892}{.009} & \seedpm{0.896}{.005} & \seedpm{0.899}{.005} & \seedpm{0.898}{.003} \\
        count exact $\uparrow$ & \seedpm{0.908}{.007} & \seedpm{0.909}{.005} & \seedpm{0.910}{.004} & \seedpm{0.909}{.004} & \seedpm{0.908}{.002} \\
        \addlinespace[2pt]
        $\mu$ MAE $\downarrow$    & \seedpm{1.71}{.07} & \seedpm{1.72}{.06} & \seedpm{1.69}{.04} & \seedpm{1.65}{.02} & \seedpm{1.67}{.03} \\
        $\sigma$ MAE $\downarrow$ & \seedpm{0.774}{.011} & \seedpm{0.778}{.011} & \seedpm{0.771}{.018} & \seedpm{0.754}{.007} & \seedpm{0.761}{.005} \\
        $a$ MAE $\downarrow$      & \seedpm{0.182}{.004} & \seedpm{0.182}{.005} & \seedpm{0.182}{.003} & \seedpm{0.187}{.005} & \seedpm{0.184}{.004} \\
        peak err mean $\downarrow$ & \seedpm{1.231}{.016} & \seedpm{1.216}{.008} & \seedpm{1.221}{.010} & \seedpm{1.213}{.007} & \seedpm{1.221}{.010} \\
        \bottomrule
      \end{tabular}
      \par\vspace{3pt}
      ...
      full norm ladder $\cdot$ hv flips $\cdot$ patch $64{\times}32$; loss $=$
      param-L1 $+\,0.5\,$cosine curve $+$ term at $w$ (column $=$ each family's
      best-scoring weight); five seeded replicas per arm, cells mean $\pm$ seed std;
      held-out test, $5.0$\,M px. Curve MAE and three-axis SSIM are flat across the
      five arms (MAE 0.0365--0.0370; SSIM elev/range/azim 0.9671--0.9675 /
      0.9510--0.9521 / 0.9610--0.9616). \textbf{bold} $=$ best in row, marked only
      where best and worst differ by more than $5\%$ --- only the pixel-$R^2$ tail
      rows clear it; every other row sits inside combined seed noise. Shading keys
      each block to its like-coloured bullet. recall / precision / count matched
      (Hungarian pairing); pixel $R^2$ min in thousands (k); $\mu$, $\sigma$ and peak
      err in elevation units, $a$ in linear amplitude; cos med: median per-pixel
      cosine to the GT profile.\par}
    \end{column}
  \end{columns}
\end{frame}
