  ...
  \begin{columns}[c]
    \begin{column}{0.56\textwidth}
      ...
      \begin{tikzpicture}[font=\scriptsize]
        ...
        \foreach \i in {1,...,29} \fill[soft!70] (0.25,{-1.6+0.1185*(\i-1)}) circle (0.030);
        \foreach \i in {1,8,15,22,29} \fill[accent] (0.25,{-1.6+0.1185*(\i-1)}) circle (0.055);
        \node[font=\tiny,text=soft,anchor=north west] at (-0.30,-1.72) {\textcolor{accent}{$\bullet$} $=$ the 5 kept};
        \node[gate,minimum height=7mm] (cfull) at (2.85,1.35) {Capon\\29 passes};
        \node[gate,minimum height=7mm] (cred)  at (2.85,0.00) {Capon\\5 passes};
        \node[box,fill=accent!16,minimum height=7mm] (net) at (2.85,-1.35) {network\\5 passes};
        \draw[flow,soft] (0.50,1.35) -- (cfull.west);
        \draw[flow] (0.50,0.00) -- (cred.west);
        \draw[flow] (0.50,-1.35) -- (net.west);
        \draw[soft,thin,fill=black!2] (4.45,0.96) rectangle ++(1.05,0.78);
        \draw[soft,thin] (4.53,1.08) -- (5.42,1.08);
        \draw[accent,semithick] plot[domain=4.53:5.42,samples=60] (\x,{1.08+0.40*exp(-(\x-4.78)^2/0.010)+0.25*exp(-(\x-5.14)^2/0.006)});
        \draw[flow,soft] (cfull.east) -- (4.45,1.35);
        \node[font=\tiny,text=good,anchor=west] at (5.54,1.35) {the reference};
        \draw[soft,thin,fill=black!2] (4.45,-0.39) rectangle ++(1.05,0.78);
        \draw[soft,thin] (4.53,-0.27) -- (5.42,-0.27);
        \draw[soft!80,densely dashed,thin] (4.78,-0.27) -- (4.78,0.24);
        \draw[soft!80,densely dashed,thin] (5.14,-0.27) -- (5.14,0.24);
        \draw[accent,semithick] plot[domain=4.53:5.42,samples=70] (\x,{-0.27+0.24*exp(-(\x-4.96)^2/0.050)+0.07*exp(-(\x-4.60)^2/0.004)+0.07*exp(-(\x-5.32)^2/0.004)});
        \draw[flow] (cred.east) -- (4.45,0.00);
        \node[font=\tiny,text=accent,anchor=west,align=left] at (5.54,0.00) {$\neq$ scatterers merge,\\\phantom{$\neq$ }sidelobes appear};
        \draw[soft,thin,fill=black!2] (4.45,-1.74) rectangle ++(1.05,0.78);
        \draw[soft,thin] (4.53,-1.62) -- (5.42,-1.62);
        \draw[accent,semithick] plot[domain=4.53:5.42,samples=60] (\x,{-1.62+0.40*exp(-(\x-4.78)^2/0.010)+0.25*exp(-(\x-5.14)^2/0.006)});
        \draw[flow] (net.east) -- (4.45,-1.35);
        \node[font=\tiny,text=good,anchor=west] at (5.54,-1.35) {$\approx$ the reference};
      \end{tikzpicture}
      \\[9pt]
      \begin{tikzpicture}[font=\scriptsize]
        ...
        \fill[accent!12] (0.75,0.82) -- (0.88,0.02) -- (1.30,0.30) -- cycle;
        \draw[accent!50,very thin] (0.75,0.82) -- (0.88,0.02);
        \draw[accent!50,very thin] (0.75,0.82) -- (1.30,0.30);
        \draw[soft,dotted,thin] (0.75,0.80) -- (0.75,-0.08);
        \fill[soft] (0.75,-0.08) circle (0.022);
        \foreach \o in {-0.06,-0.03,0,0.03,0.06} \draw[ink!50,very thin] (-0.1,{0.86+\o}) -- (2.3,{0.86+\o});
        \fill[ink!70] (0.75,0.86) -- ++(-0.15,0.055) -- ++(0.045,-0.055) -- ++(-0.045,-0.055) -- cycle;
        \node[font=\tiny,text=soft,anchor=west] at (2.42,0.86) {flight line: 29 passes};
        \node[font=\tiny,text=accent!80,anchor=west] at (1.45,0.55) {side-looking beam};
        \node[font=\tiny,text=soft,anchor=north] at (1.7,-0.14) {one scene};
        \node[font=\scriptsize\bfseries,text=accent,anchor=west] at (3.35,0.30) {$\times\,5.8$ area};
        \foreach \x in {0.5,1.48,2.46,3.44,4.42} {
          \draw[soft,thin,rounded corners=1pt,fill=black!2] (\x,-1.85) -- ++(0.88,0) -- ++(0.42,0.28) -- ++(-0.88,0) -- cycle;
          \foreach \f in {0.33,0.66} \draw[black!7,very thin] ({\x+\f*0.42},{-1.85+\f*0.28}) -- ({\x+0.88+\f*0.42},{-1.85+\f*0.28});
          \foreach \g in {0.33,0.66} \draw[black!7,very thin] ({\x+\g*0.88},-1.85) -- ({\x+\g*0.88+0.42},-1.57);
        }
        \draw[soft,thin,densely dashed] (5.40,-1.85) -- ++(0.70,0) -- ++(0.42,0.28) -- ++(-0.70,0) -- cycle;
        \fill[accent!20] (5.41,-1.85) -- (5.55,-1.85) -- (5.97,-1.57) -- (5.83,-1.57) -- cycle;
        \fill[accent!12] (5.35,-0.88) -- (5.48,-1.83) -- (5.90,-1.55) -- cycle;
        \draw[accent!50,very thin] (5.35,-0.88) -- (5.48,-1.83);
        \draw[accent!50,very thin] (5.35,-0.88) -- (5.90,-1.55);
        \draw[soft,dotted,thin] (5.35,-0.90) -- (5.35,-1.93);
        \fill[soft] (5.35,-1.93) circle (0.022);
        \foreach \o in {-0.025,0.025} \draw[ink!50,very thin] (-0.1,{-0.84+\o}) -- (6.0,{-0.84+\o});
        \fill[ink!70] (5.35,-0.84) -- ++(-0.15,0.055) -- ++(0.045,-0.055) -- ++(-0.045,-0.055) -- cycle;
        \node[font=\tiny,text=soft,anchor=south west] at (0,-0.78) {flight line: 5 passes per scene};
        \node[font=\tiny,text=soft,anchor=north] at (3.3,-1.97) {$29/5 \approx 5.8$ scenes};
        \draw[decorate,decoration={brace,amplitude=4pt},soft] (-0.35,-1.85) -- (-0.35,0.92);
        \node[font=\tiny,text=soft,rotate=90,anchor=south] at (-0.57,-0.465) {same flight time};
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.40\textwidth}
      \footnotesize
      Elevation resolution is bought with flight passes: every pass adds one
      vertical baseline sample over the scene.
      \par\smallskip
      \begin{itemize}\setlength{\itemsep}{7pt}
        \item \textbf{The problem}: on the reduced stack the classical inversion
              degrades: with 5 baselines nearby scatterers merge and sidelobes
              contaminate the profile.
        \item \textbf{The goal}: a network trained against full-stack ground
              truth recovers the full-stack parameters $[a_k,\mu_k,\sigma_k]$
              from the 5-pass stack alone.
        \item \textbf{The payoff}: a scene then costs 5 passes instead of 29,
              so the same airtime surveys $29/5 \approx 5.8$ scenes.
        \item[\textcolor{soft}{$\blacksquare$}] \textbf{The question}: how close
              does the reconstruction come to the reference? The rest of the
              talk measures that.
      \end{itemize}
    \end{column}
  \end{columns}
