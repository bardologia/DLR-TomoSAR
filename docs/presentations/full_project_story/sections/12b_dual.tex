...
% ---------------------------------------------------------------- 25aa Pair sweep at K=2: the board
\begin{frame}{The pair sweep at $K{=}2$ --- the curve term rides free, cosine shifts the operating point}
    \centering
  \scriptsize
  \renewcommand{\arraystretch}{0.85}
  \setlength{\tabcolsep}{2.5pt}
  \renewcommand{\seedtag}{plA}
  \begin{tabular}{@{}l ccccccccc@{}}
    ...
    \quad $w{=}1$ & \seedpm{0.749}{.003} & \seedpm{0.966}{.002} & \seedpm{0.884}{.010} & \seedpm{0.902}{.002} & \seedpm{0.893}{.005} & \seedpm{0.900}{.011} & \seedpm{\hlval{good}{\textbf{0.037}}}{.003} & \seedpm{\hlval{accent}{0.064}}{.013} & \seedpm{\textbf{4.7}}{.0} \\
    \bottomrule
    \end{tabular}
  \par\vspace{3pt}
  \parbox{0.92\textwidth}{\tiny\color{soft} \textbf{runs}: dual UNet-skip trunks $\cdot$ set-pred $\cdot$ Hungarian $\cdot$ $K{=}2$ $\cdot$ presence A $\cdot$ full norm ladder $\cdot$ hv flips $\cdot$ patch $64{\times}32$ $\cdot$ fixed routing (full stack $\to$ both trunks) $\cdot$ parity twin ladders (${\approx}31.19$\,M). Row $=$ objective $(\mathcal{L}_{\text{param-L1}}+w\,\mathcal{L}_{\text{curve}})/(1+w)$. Five seeded replicas per arm, cells mean $\pm$ seed std; held-out test, $5.0$\,M px. Validation losses are omitted --- every arm optimizes a different objective, so they do not compare. Curve MAE, PSNR and three-axis SSIM are flat over all sixteen arms (0.036--0.037, 52.6--52.8\,dB, elev/range/azim 0.967--0.968\,/\,0.951--0.952\,/\,0.961--0.962). \textbf{bold} $=$ best in column, marked only where best and worst differ by more than $5\%$. \textcolor{good}{Green} and \textcolor{accent}{red} sit on one row: full-weight cosine trades misses for false alarms (under 0.049 $\to$ 0.037, over 0.040 $\to$ 0.064) at flat F1 --- an operating-point shift, not extra detection. Every other column stays inside combined seed noise: at $K{=}2$ the pair term rides along free. cos med: median per-pixel cosine to the GT profile; peak p95: 95th percentile of the per-pixel peak error.\par}
\end{frame}

% ---------------------------------------------------------------- 25ab Pair at K=2 by scatterer count
\begin{frame}{The $K{=}2$ pair by scatterer count --- wider firing buys the second scatterer, position pays}
    \centering
  \scriptsize
  \renewcommand{\arraystretch}{0.92}
  \setlength{\tabcolsep}{6pt}
  \renewcommand{\seedtag}{plB}
  \begin{tabular}{@{}l cccccc@{}}
    \toprule
    & \multicolumn{6}{c@{}}{paired curve term} \\
    \cmidrule(l){2-7}
    metric & none & L1 $w{=}1$ & Charb.\ $w{=}1$ & cos.\ $w{=}0.01$ & cos.\ $w{=}0.5$ & cos.\ $w{=}1$ \\
    \midrule
    ...
    \quad $k{=}2$ & \seedpm{0.742}{.006} & \seedpm{0.737}{.005} & \seedpm{0.735}{.006} & \seedpm{0.755}{.003} & \seedpm{0.764}{.005} & \seedpm{\hlval{good}{0.765}}{.004} \\
    \addlinespace
    \multicolumn{7}{@{}l}{\emph{count acc $\mid$ pred $k$}} \\
    ...
    \addlinespace
    $\mu$ MAE $\downarrow$ & \seedpm{\textbf{1.59}}{.02} & \seedpm{1.61}{.01} & \seedpm{1.60}{.02} & \seedpm{1.68}{.04} & \seedpm{1.69}{.04} & \seedpm{\hlval{accent}{1.73}}{.04} \\
    ...
    \addlinespace
    $\sigma$ MAE $\downarrow$ & \seedpm{0.690}{.008} & \seedpm{0.690}{.008} & \seedpm{\textbf{0.686}}{.005} & \seedpm{0.749}{.024} & \seedpm{0.765}{.005} & \seedpm{\hlval{accent}{0.783}}{.013} \\
    ...
    \end{tabular}
  \par\vspace{1pt}
  \parbox{0.82\textwidth}{\tiny\color{soft} \textbf{runs}: dual UNet-skip trunks $\cdot$ set-pred $\cdot$ Hungarian $\cdot$ $K{=}2$ $\cdot$ presence A $\cdot$ full norm ladder $\cdot$ hv flips $\cdot$ patch $64{\times}32$ $\cdot$ fixed routing (full stack $\to$ both trunks) $\cdot$ parity twins. Column $=$ paired curve term at its weight $w$; none $=$ param-L1 alone. All rows matched (Hungarian pairing); cells mean $\pm$ seed std over five seeded replicas; held-out test; $k$ $=$ true scatterer count of the pixel; component errors over matched pairs only. \textbf{bold} $=$ best in row, marked only where best and worst differ by more than $5\%$. \textcolor{good}{Green} $=$ the cosine arms' two-scatterer recall (0.742 $\to$ 0.765, ${\approx}3\times$ combined seed noise); \textcolor{accent}{red} $=$ what the wider gate costs: $\mu$ and $\sigma$ drift up with cosine weight ($\mu$ 1.59 $\to$ 1.73, $\sigma$ 0.690 $\to$ 0.783, both past noise) while $k{=}1$ precision slips inside the gate (0.947 $\to$ 0.920). The magnitude pairs move nothing anywhere. Precision rewards under-prediction --- rank on recall. count acc $\mid$ pred $k$: share of pixels claiming exactly $k$ scatterers whose true count is $k$. $\mu$, $\sigma$ in elevation units; $a$ in linear amplitude.\par}
\end{frame}

% ---------------------------------------------------------------- 25ac Pair at K=2: predicted stats vs GT
\begin{frame}{The $K{=}2$ pair, predicted statistics --- the gate fires wider, leakage thins}
    \centering
  \scriptsize
  \renewcommand{\arraystretch}{0.90}
  \setlength{\tabcolsep}{4pt}
  \renewcommand{\seedtag}{plC}
  \begin{tabular}{@{}l !{\color{white}\vrule width 2pt} c !{\color{white}\vrule width 2pt} cccccc@{}}
    \toprule
    & & \multicolumn{6}{c@{}}{paired curve term} \\
    \cmidrule(l){3-8}
    mean & \textcolor{good}{GT} & none & L1 $w{=}1$ & Charb.\ $w{=}1$ & cos.\ $w{=}0.01$ & cos.\ $w{=}0.5$ & cos.\ $w{=}1$ \\
    \midrule
    ...
    \end{tabular}
  \par\vspace{3pt}
  \parbox{0.82\textwidth}{\tiny\color{soft} \textbf{runs}: dual UNet-skip trunks $\cdot$ set-pred $\cdot$ Hungarian $\cdot$ $K{=}2$ $\cdot$ presence A $\cdot$ full norm ladder $\cdot$ hv flips $\cdot$ patch $64{\times}32$ $\cdot$ fixed routing (full stack $\to$ both trunks) $\cdot$ parity twins. Column $=$ paired curve term at its weight $w$; none $=$ param-L1 alone. Five seeded replicas per arm, cells mean $\pm$ seed std; means conditioned on the slot's GT activity mask. \textbf{bold} $=$ closest to the \textcolor{good}{GT} column, marked only where best and worst differ by more than $5\%$. The cosine gate fires wider on both slots (slot 0: 0.972 $\to$ 0.988; slot 1: 0.268 $\to$ 0.299 against GT 0.260 --- past the GT into over-fire), thins the inactive-amplitude leakage (0.85 $\to$ 0.57) and re-widens the compressed $\sigma$ statistics toward the GT (slot-0 $\sigma$ width 1.20 $\to$ 1.38 vs 2.84) --- the shape term pushes the predicted distributions outward at $K{=}2$ too, it just finds nothing new to detect. Inactive GT slots define no $\mu$/$\sigma$ reference (---). Width rows: std of the prediction over GT-active pixels vs the GT's own std. $\mu$, $\sigma$ in elevation units; $a$ in linear amplitude.\par}
\end{frame}

% ---------------------------------------------------------------- 25ad Pair sweep at K=5: the board
\begin{frame}{The pair sweep at $K{=}5$ --- a shape term at trace weight wakes the gate}
    \centering
  \scriptsize
  \renewcommand{\arraystretch}{0.85}
  \setlength{\tabcolsep}{2.5pt}
  \renewcommand{\seedtag}{pkA}
  \begin{tabular}{@{}l ccccccccc@{}}
    \toprule
    & \multicolumn{2}{c}{reconstruction} & \multicolumn{7}{c@{}}{detection (matched)} \\
    \cmidrule(lr){2-3}\cmidrule(l){4-10}
    ...
    \addlinespace
    \multicolumn{10}{@{}l}{\emph{$+$ cosine curve}} \\
    \quad $w{=}0.01$ & \seedpm{0.683}{.011} & \seedpm{0.949}{.005} & \seedpm{\textbf{0.918}}{.007} & \seedpm{\hlval{good}{0.813}}{.003} & \seedpm{\textbf{0.863}}{.002} & \seedpm{\hlval{good}{\textbf{0.810}}}{.003} & \seedpm{\hlval{good}{0.142}}{.004} & \seedpm{0.048}{.004} & \seedpm{5.1}{.4} \\
    ...
    \quad $w{=}1$ & \seedpm{0.700}{.005} & \seedpm{0.954}{.002} & \seedpm{\hlval{accent}{0.867}}{.009} & \seedpm{\textbf{0.843}}{.001} & \seedpm{0.855}{.004} & \seedpm{0.794}{.010} & \seedpm{\textbf{0.104}}{.002} & \seedpm{\hlval{accent}{0.102}}{.011} & \seedpm{\textbf{4.7}}{.0} \\
    \bottomrule
    \end{tabular}
  \par\vspace{3pt}
  \parbox{0.92\textwidth}{\tiny\color{soft} \textbf{runs}: dual UNet-skip trunks $\cdot$ set-pred $\cdot$ Hungarian $\cdot$ $K{=}5$ $\cdot$ presence A $\cdot$ full norm ladder $\cdot$ hv flips $\cdot$ patch $64{\times}32$ $\cdot$ fixed routing (full stack $\to$ both trunks) $\cdot$ parity twin ladders. Row $=$ objective $(\mathcal{L}_{\text{param-L1}}+w\,\mathcal{L}_{\text{curve}})/(1+w)$. Five seeded replicas per arm, cells mean $\pm$ seed std; held-out test, $K_{\max}{=}5$ labels. Validation losses are omitted --- every arm optimizes a different objective, so they do not compare. Curve MAE, PSNR and three-axis SSIM barely move (0.044--0.046, 51.0--51.5\,dB, elev/range/azim 0.961--0.963\,/\,0.940--0.947\,/\,0.954--0.957); curve $R^2$ rises with any strong curve term (0.680 $\to$ 0.712, ${\approx}3\times$ seed noise, under the 5\% bold gate). \textbf{bold} $=$ best in column past $5\%$. \textcolor{good}{Green} $=$ the trace-weight cosine arm: recall $+0.14$, count exact $+0.18$, under-count 0.33 $\to$ 0.14 --- while holding the board's best precision. \textcolor{accent}{Red} $=$ the price of pushing $w$: full-weight cosine pays precision (0.867) and over-fires (0.102). The magnitude terms buy the same wake-up only at $w{=}1$, and less of it. cos med: median per-pixel cosine to the GT profile; peak p95: 95th percentile of the per-pixel peak error.\par}
\end{frame}

% ---------------------------------------------------------------- 25ae Pair at K=5 by scatterer count
\begin{frame}{The $K{=}5$ pair by scatterer count --- the missing scatterers were single}
    \centering
  \scriptsize
  \renewcommand{\arraystretch}{1.0}
  \setlength{\tabcolsep}{6pt}
  \renewcommand{\seedtag}{pkB}
  \begin{tabular}{@{}l cccccc@{}}
    \toprule
    & \multicolumn{6}{c@{}}{paired curve term} \\
    \cmidrule(l){2-7}
    metric & none & L1 $w{=}1$ & Charb.\ $w{=}1$ & cos.\ $w{=}0.01$ & cos.\ $w{=}0.5$ & cos.\ $w{=}1$ \\
    \midrule
    precision $\uparrow$ & \seedpm{0.912}{.007} & \seedpm{0.917}{.009} & \seedpm{0.915}{.012} & \seedpm{\textbf{0.918}}{.007} & \seedpm{0.883}{.010} & \seedpm{\hlval{accent}{0.867}}{.009} \\
    ...
    \quad $k{=}1$ & \seedpm{0.759}{.024} & \seedpm{0.975}{.006} & \seedpm{0.962}{.029} & \seedpm{\hlval{good}{0.995}}{.001} & \seedpm{\textbf{0.997}}{.000} & \seedpm{\textbf{0.997}}{.000} \\
    ...
    \end{tabular}
  \par\vspace{3pt}
  \parbox{0.82\textwidth}{\tiny\color{soft} \textbf{runs}: dual UNet-skip trunks $\cdot$ set-pred $\cdot$ Hungarian $\cdot$ $K{=}5$ $\cdot$ presence A $\cdot$ full norm ladder $\cdot$ hv flips $\cdot$ patch $64{\times}32$ $\cdot$ fixed routing (full stack $\to$ both trunks) $\cdot$ parity twins. Column $=$ paired curve term at its weight $w$; none $=$ param-L1 alone. All precision / recall rows matched (Hungarian pairing); cells mean $\pm$ seed std over five seeded replicas; held-out test; $k$ $=$ true scatterer count of the pixel. \textbf{bold} $=$ best in row past $5\%$. \textcolor{good}{Green}: the baseline's misses were mostly single scatterers --- $k{=}1$ recall 0.759 $\to$ 0.995 at trace-weight cosine (0.975/0.962 for the magnitude terms at $w{=}1$); the deep rungs move far less ($k{=}5$ 0.246 $\to$ 0.308, heavy cosine only). \textcolor{accent}{Red} $=$ the full-weight cosine precision price. Claimed counts $\geq 2$ stay unreliable in every arm (pred-2 accuracy $\leq$ 0.50). Precision rewards under-prediction --- rank on recall. count acc $\mid$ pred $k$: share of pixels claiming exactly $k$ scatterers whose true count is $k$.\par}
\end{frame}

% ---------------------------------------------------------------- 25af Pair at K=5: component errors
\begin{frame}{The $K{=}5$ pair components --- magnitude pairs localise, heavy cosine pays position}
    \centering
  \scriptsize
  \renewcommand{\arraystretch}{1.0}
  \setlength{\tabcolsep}{6pt}
  \renewcommand{\seedtag}{pkC}
  \begin{tabular}{@{}l cccccc@{}}
    \toprule
    & \multicolumn{6}{c@{}}{paired curve term} \\
    \cmidrule(l){2-7}
    metric & none & L1 $w{=}1$ & Charb.\ $w{=}1$ & cos.\ $w{=}0.01$ & cos.\ $w{=}0.5$ & cos.\ $w{=}1$ \\
    \midrule
    $\mu$ MAE $\downarrow$ & \seedpm{1.21}{.07} & \seedpm{\hlval{good}{\textbf{1.10}}}{.04} & \seedpm{\hlval{good}{\textbf{1.10}}}{.04} & \seedpm{1.13}{.05} & \seedpm{1.33}{.06} & \seedpm{\hlval{accent}{1.42}}{.06} \\
    ...
    \addlinespace
    $a$ MAE $\downarrow$ & \seedpm{0.213}{.005} & \seedpm{0.204}{.006} & \seedpm{0.206}{.009} & \seedpm{\hlval{good}{\textbf{0.168}}}{.003} & \seedpm{0.190}{.007} & \seedpm{0.203}{.006} \\
    ...
    \end{tabular}
  \par\vspace{3pt}
  \parbox{0.82\textwidth}{\tiny\color{soft} \textbf{runs}: dual UNet-skip trunks $\cdot$ set-pred $\cdot$ Hungarian $\cdot$ $K{=}5$ $\cdot$ presence A $\cdot$ full norm ladder $\cdot$ hv flips $\cdot$ patch $64{\times}32$ $\cdot$ fixed routing (full stack $\to$ both trunks) $\cdot$ parity twins. Column $=$ paired curve term at its weight $w$; none $=$ param-L1 alone. Cells mean $\pm$ seed std over five seeded replicas; held-out test; $k$ $=$ true scatterer count of the pixel; component errors over matched pairs only --- an arm that recalls more matches a harder pool, so read this frame with the recall frame. \textbf{bold} $=$ best in row past $5\%$. \textcolor{good}{Green} $=$ the magnitude pairs cut $\mu$ despite matching far more pairs ($\mu$ 1.21 $\to$ 1.10, ${\approx}2\times$ noise), and trace-weight cosine takes amplitude on every rung ($a$ MAE 0.213 $\to$ 0.168). \textcolor{accent}{Red} $=$ heavy cosine pays position: $\mu$ 1.42 overall, 2.60 $\to$ 3.69 at $k{=}3$ --- the ladder steepens as $w$ grows. $\mu$, $\sigma$ in elevation units; $a$ in linear amplitude.\par}
\end{frame}

% ---------------------------------------------------------------- 25ag Pair at K=5: predicted stats vs GT
\begin{frame}{The $K{=}5$ pair, predicted statistics --- slot 0 comes back on}
    \centering
  \scriptsize
  \renewcommand{\arraystretch}{0.78}
  \setlength{\tabcolsep}{4pt}
  \renewcommand{\seedtag}{pkD}
  \begin{tabular}{@{}l !{\color{white}\vrule width 2pt} c !{\color{white}\vrule width 2pt} cccccc@{}}
    \toprule
    & & \multicolumn{6}{c@{}}{paired curve term} \\
    \cmidrule(l){3-8}
    ...
    \; slot 0 & 1.000 & \seedpm{0.756}{.018} & \seedpm{0.930}{.011} & \seedpm{0.921}{.022} & \seedpm{\hlval{good}{0.948}}{.007} & \seedpm{0.964}{.003} & \seedpm{\textbf{0.967}}{.001} \\
    ...
    \end{tabular}
  \par\vspace{2pt}
  \parbox{0.82\textwidth}{\tiny\color{soft} \textbf{runs}: dual UNet-skip trunks $\cdot$ set-pred $\cdot$ Hungarian $\cdot$ $K{=}5$ $\cdot$ presence A $\cdot$ full norm ladder $\cdot$ hv flips $\cdot$ patch $64{\times}32$ $\cdot$ fixed routing (full stack $\to$ both trunks) $\cdot$ parity twins. Column $=$ paired curve term at its weight $w$; none $=$ param-L1 alone. Five seeded replicas per arm, cells mean $\pm$ seed std; means conditioned on the slot's GT activity mask. \textbf{bold} $=$ closest to the \textcolor{good}{GT} column, marked only where best and worst differ by more than $5\%$. \textcolor{good}{Green} $=$ the wake-up: slot 0 fires on 0.756 of pixels under param-L1 alone and on 0.948 with the trace-weight cosine term (0.96--0.97 at heavier $w$, GT 1.000); slot-1 activity climbs toward the GT (0.177 $\to$ 0.218--0.228 vs 0.239) and the slot-0 amplitude bias clears (0.92 $\to$ 0.75--0.77 vs GT 0.84). Deep-slot activity stays under the GT in every arm --- the wake-up is a slot-0/1 story. Width rows: std of the prediction over GT-active pixels vs the GT's own std, shown for slots 0--1 (tail slots in the report). $\mu$, $\sigma$ in elevation units; $a$ in linear amplitude.\par}
\end{frame}
