...
- PHYSICS SINGLE-TERM SWEEP RESULTS LANDED 2026-08-06 (user: emailed results "for the
  ...
  SECOND FOLLOW-UP same day (user, caps: "LOOK AT THE PAIR LOSS FUNCTIONS RESULTS
  AS A EXAMPLE, THERE ARE NO BULLET POINTS, ITS CENTRALIZED, THERE ARE MANY TABLES
  WITH VERY DETAILED RESULTS, PLEASE, JUST KEEP IT LIKE THAT"): both bullets+table
  frames were REPLACED by THREE pair-block-style frames --- centered, no bullet
  columns, dense \seedpm tables mirroring 25aa/25ab: (a) "The physics sweep at
  K=2 --- every term rides low, the scale terms collapse high" = the full 21-arm
  board (param-L1+cos reference row, then 4 family groups x 5 weights as \quad w=
  rows) with the exact 25aa column set (curve R2 / cos med | prec / recall / F1 /
  exact / under / over / peak p95), bold best-in-COLUMN under the 5% gate, hlval
  green on coherence w0.25 curve R2 + the w1 under/over operating-point pair
  (0.041->0.032 / 0.051->0.083, seed lottery prec +-.045 exact +-.063), red on the
  four covariance/total-power collapse R2 cells; arraystretch 0.70, tabcolsep
  2.2pt, caption compressed over four passes (a residual ~6pt vbox overfull is
  tolerated --- page rastered, nothing clips; the seedpm strut floor makes
  arraystretch below ~0.76 nearly free of effect). (b) "by scatterer count" (25ab
  pattern): 6 columns none / coh .25 / coh 1 / mom .05 / cov .05 / pow .05;
  precision / recall / count-acc-pred-k / mu / sigma / a with k=1/k=2 sub-rows ---
  almost nothing clears the 5% gate (story: the scatterer set is untouched;
  full-weight coherence pays mu/sigma and destabilises claimed pairs +-.122). (c)
  "per-pixel tail": curve-level / three-axis SSIM / pixel-R2
  median-mean-p5-p1-min / pixel-cosine blocks --- the tail-insurance story lives
  here, and the board shows coherence is NOT tail insurance (w1 tail WORSE than
  baseline: mean -3.71, min -22.5k). ALL table rows GENERATED by scratchpad
  gen_physics_tables.py (parses metrics_comparison.md, emits seedpm rows, computes
  the 5% gate per column/row --- never hand-type dense tables, the mega2.py
  lesson). The grouped-score table frame is GONE --- raw-metric boards replaced the
