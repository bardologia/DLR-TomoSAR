...
`serve.py` also takes `--host`, default `127.0.0.1`.
...
| Scripts | `#/scripts` | `/api/scripts` | The entry points as cards. `ScriptCatalog` describes 39 of them and folds the per-family variants into 4 groups (train, infer, analyze, compare); `main/` holds 41 entry scripts across `processing/`, `training/`, `inference/`, `analysis/` and `experiments/`. |
...
| Microscope | `#/microscope` | `/api/probe/*` | Model microscope over a trained run: load the checkpoint with its real dataset, click a pixel, and inspect the predicted profile against ground truth and the raw tomogram, per-channel gradient attribution with a saliency map, per-layer feature-map grids and live what-if channel perturbations. |
...
and 34 modules under `static/js/`. `app.js` and `router.js` wire the views; one module per
tab (`home`, `equations`, `flow_view` with `flow_sketches`, `physics_loss`, `pipelines`,
`repomap`, `models`, `scripts`, `launch` with `launch_widgets` and `launch_pickers`,
`ablation`, `saved_runs`, `configs`, `console`, `tensorboard`, `leaderboard`, `results`,
`tomogram`, `slice_collector`, `fit_lab`, `microscope`, `triage`, `autopsy`,
`feed_tuner`); the rest are shared drawing and animation helpers (`radar`, `diagram`,
`gauges`, `process_anim`, `server_anim`, `tomo_anim`).
