...
| Leaderboard | `/api/leaderboard` | Three modes. Runs: every saved inference `metrics.json` as one sortable, filterable table with run-name axes parsed into filter dropdowns and a two-row metric plus resolved-config diff; seeded `<unit>/seed<N>/` layouts collapse to one seed-mean unit row per unit (latest stamp per seed, individual seeds never listed), selectable into the comparison like any stamp row. Trials: seeded runs aggregated to mean-and-std bars per experiment. Curves: native overlay of tensorboard scalar curves across selected training runs. |
