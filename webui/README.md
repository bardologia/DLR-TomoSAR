...
| Leaderboard | `/api/leaderboard` | Three modes. Runs: every saved inference `metrics.json` as one sortable, filterable table with run-name axes parsed into filter dropdowns and a two-row metric plus resolved-config diff. Trials: seeded `<unit>/seed<N>/` runs aggregated to mean-and-std bars per experiment. Curves: native overlay of tensorboard scalar curves across selected training runs. |
