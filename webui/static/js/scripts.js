...
class ScriptPanel {
  ...
  _card(item) {
    ...
    const variants = item.variants.map((v) =>
      `<span class="script-card__variant" data-key="${v.key}">${this._esc(v.label)}</span>`
    ).join("");

    card.innerHTML =
      `<span class="script-card__glow"></span>` +
      `<div class="script-card__top"><span class="script-card__cat">${this._esc(item.category)}</span>` +
      `<span class="script-card__file">${this._esc(item.file)}</span></div>` +
      `<h3 class="script-card__title">${this._esc(item.title)}</h3>` +
      `<p class="script-card__purpose" title="${this._esc(item.purpose)}">${this._esc(item.purpose)}</p>` +
      (variants ? `<div class="script-card__variants">${variants}</div>` : "") +
      `<div class="script-card__foot"><span>${this._esc(item.foot)}</span>` +
